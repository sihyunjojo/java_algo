...
public class Bj_G3_20058_마법사상어와파이어스톰_Stranger {
    static int[][] board;
...
//        int[] magics = new int[q];
    ...
    static void turn(int num) {
        ...
        int two_pow_mini = (int) Math.pow(2, num - 1);
        int[][] delta = new int[][]{{0, two_pow_mini}, {two_pow_mini, 0}, {-two_pow_mini, 0},{0, -two_pow_mini}};
        ...
        for (int i = 0; i < two_pow_n; i = i + two_pow) {
            for (int j = 0; j < two_pow_n; j = j + two_pow) {
                ...
                int index = 0;
                for (int k = i; k < two_pow + i; k += two_pow_mini) {
                    for (int l = j; l < two_pow + j; l += two_pow_mini) {
//                        if (!one_flag){
                        temp[index][0] = k;
                        temp[index++][1] = l;
//                    }

//                        if (one_flag) temp_board[k][j+1- l] = board[l][k]; // 외워
                    }
                }

//                if (!one_flag) {
                for (int k = 0; k < 4; k++) {
                    for (int l = temp[k][0]; l < temp[k][0] + two_pow_mini; l++) {
                        for (int ll = temp[k][1]; ll < temp[k][1] + two_pow_mini; ll++) {
                            temp_board[l + delta[k][0]][ll + delta[k][1]] = board[l][ll];
                        }
                    }
                }
//                }
//                for (int[] ints : temp_board) {
...
//                        if (!check_out_of_index(dy,dx)) continue;
